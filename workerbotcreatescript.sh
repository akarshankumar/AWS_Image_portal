...
aws elb create-lb-cookie-stickiness-policy --load-balancer-name $ELBURL --policy-name MyWorkerBOTStickyPolicy --cookie-expiration-period 300
...
aws ec2 run-instances --image-id ami-30a42358 --count $3 --region us-east-1 --instance-type t1.micro --key-name $1 --security-group-ids $SGID --subnet-id $subnet --associate-public-ip-address --client-token $2 --output=text --user-data file://setup-FP_workerBOT.sh --iam-instance-profile Name=$6 --block-device-mappings "[{\"DeviceName\": \"/dev/sdh\",\"Ebs\":{\"VolumeSize\":10}}]"
